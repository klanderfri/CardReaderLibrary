...
public:
	...
	static cv::Scalar ToScalarColour(const Colour colour);
	...
	static cv::Mat DrawCenterPoint(const cv::Mat image, const cv::Point imageCenter, const Colour colour = Red, int radius = 3);
	//Fills a contour in the image with the specified colour.
	static void FillContour(cv::Mat& image, const Contour contour, const Colour colour);
