...
private:
	...
	FigureArea getCardArea(const cv::Mat workingImage, const cv::Size originalImageSize);
	//Extracts the cropped image of the card from the original card image.
	cv::Mat getCardImage(const cv::Mat rawImage, const FigureArea cardArea);
	...
	FigureArea findCardArea(const cv::Mat rawImage, int thresh);
